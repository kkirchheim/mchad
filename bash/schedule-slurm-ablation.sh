...
options="++hydra.launcher.ray.remote.num_gpus=0.25 ++hydra.launcher.ray.remote.num_cpus=2 ++hydra.launcher.ray.init.num_cpus=80 ++hydra.launcher.ray.init.num_gpus=4 +trainer.val_check_interval=0.10 $@"

srun python run.py -m experiment="cifar10-gmchad" model.weight_oe="range(0,1.1,0.1)" trainer.gpus=1 hydra.sweep.dir="${log_dir}/vary_oe" ${options}
srun python run.py -m experiment="cifar10-gmchad" model.weight_ce="range(0,1.1,0.1)" trainer.gpus=1 hydra.sweep.dir="${log_dir}/vary_ce" ${options}
srun python run.py -m experiment="cifar10-gmchad" model.weight_center="range(0,1.1,0.1)" trainer.gpus=1 hydra.sweep.dir="${log_dir}/vary_center" ${options}

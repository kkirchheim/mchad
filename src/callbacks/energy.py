...
class EnergyBased(pl.callbacks.Callback):
    ...
    def _eval_epoch_end(self, pl_module, stage, **kwargs):
        ...
        log_uncertainty_metrics(
            pl_module, -energy, stage, y, -energy, method=EnergyBased.NAME
        )
        log_error_detection_metrics(
            pl_module, -energy, stage, y, y_hat, method=EnergyBased.NAME
        )

        try:
            log_score_histogram(
                pl_module, stage, energy, y, y_hat, method=EnergyBased.NAME
            )
